...
async function generate(auditPath, outputPath, options = {}) {
  ...
  try {
    ...
    // Stage 6: HTML Polish Pass (manual by default, LLM only if --llm-polish)
    if (!options.skipFinalPass) {
      console.log('Stage 6: Running final HTML polish...');

      let polishResult;
      if (options.llmPolish) {
        // LLM polish: slower but more thorough (opt-in via --llm-polish)
        polishResult = await runFinalHtmlPass(htmlPath, {
        ...
        // Manual polish: fast deterministic fixes (default)
        const fs = await import('fs');
        const htmlContent = fs.default.readFileSync(htmlPath, 'utf8');
        const { html: polishedHtml, changes } = manualPolishHTML(htmlContent);
        fs.default.writeFileSync(htmlPath, polishedHtml, 'utf8');
        polishResult = { htmlPath, changes, method: 'manual' };
      }

      results.stages.htmlPolish = {
        success: true,
        method: polishResult.method,
        changes: polishResult.changes
      };

      // Log polish summary
      if (polishResult.changes?.length > 0) {
        console.log(`  Polish applied (${polishResult.method}): ${polishResult.changes.length} improvement(s)`);
      }
    } else {
      results.stages.htmlPolish = { success: true, method: 'skipped' };
    }

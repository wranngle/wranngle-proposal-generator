...
const DEFAULT_CONFIG = {
  brand: {
    brand_name: process.env.PRODUCER_NAME || 'Your Company',
    logo_uri: process.env.LOGO_URI || '',
    primary_domain: process.env.PRODUCER_DOMAIN || 'example.com'
  },
  producer: {
    producer_name: process.env.PRODUCER_NAME || 'Your Company Name',
    producer_email: process.env.PRODUCER_EMAIL || 'contact@example.com'
  },
  defaults: {
  ...
  cta: {
    book_call_link: process.env.CTA_CALENDLY_LINK || '',
...
function buildCTASection(proposalNumber, validUntil, platform, config) {
  ...
  const approveLink = config.cta.approve_link_template.replace('{proposal_number}', proposalNumber);
  ...
  return {
    ...
    secondary_action: {
      ...
      link: config.cta.book_call_link
    },
    expires_display: `Proposal valid until ${expiresDisplay}`
  };
}
